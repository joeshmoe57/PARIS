...
class OurGen:
   ...
   @staticmethod 
   def badRand(bitsLong):
      while True:
         n =  Crypto.Random.new()
         return n.read(bitsLong)

         #check our dict of used randoms. if there is one for our length, use it. else, generate a new one
         if bitsLong not in used:
            print 'was false. making new'
            used[bitsLong] = r.read(bitsLong)
         
         print str(used[bitsLong])
         return str(used[bitsLong])
...
def GenerateBadProcess(q):
   OurGen.r = Crypto.Random.new()
   #generate keys until the program ends
   while True:
      #make the key
      k = RSA.generate(1024, e=65537, randfunc=OurGen.badRand)
...
annotations = open('keys-annotations.txt', 'w')
