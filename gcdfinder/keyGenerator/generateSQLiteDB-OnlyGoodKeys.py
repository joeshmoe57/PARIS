...
if len(sys.argv) == 5:
   ...
   freqError = float(sys.argv[3])
else:
   print 'Wrong num of params. #1 is number of processes, #2 is number of keys, #3 is float percent (IE 23.3) of keys to be duplicate'
   exit(1)

if not 0 < freqError < 100:
   print 'error frequency must be between 0 and 100'
   exit(1)


print 'About to generate a db of ' + str(numKeys) + ' 1024 bit keys with ' + str(numProcesses) + ' processes.\nThere will be ' + str(freqError) + '% of the keys will be repeated\n'
